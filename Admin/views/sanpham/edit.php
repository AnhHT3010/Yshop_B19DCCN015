                ...
                <div class="tile-body">
                    ...
                    <form class="row" action="./?mod=sanpham&act=update" method="POST" role="form" enctype="multipart/form-data">
                            ...
                            <div class="col-sm-10">
                                <select class="form-select" name="MaDM" id="selectCategory" aria-label="Default select example">
                                    <?php foreach ($data_category as $row) { ?>
                                        <option <?= ($data_product['MaDM'] == $row['MaDM']) ? 'selected' : '' ?> value="<?= $row['MaDM'] ?>"><?= $row['TenDM'] ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-10">
                                <select class="form-select" name="MaLSP" id="selectBrand" aria-label="Default select example">
                                    <?php foreach ($data_brand as $row) { ?>
                                        <option value="<?= $row['MaLSP'] ?>" data-ma-dm="<?= $row['MaDM'] ?>">
                                            <?= $row['TenLSP'] ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control" name="TenSP" type="text" value="<?= $data_product['TenSP'] ?>">
                            <span class="error-message" id="tenSPError"></span>
                        </div>
                        <div class="form-group col-md-3">
                            <label class="control-label">Đơn giá<b style="color: red">*</b></label>
                            <input class="form-control" name="DonGia" type="text" value="<?= $data_product['DonGia'] ?>">
                            <span class="error-message" id="donGiaError"></span>
                        </div>
                        <div class="form-group col-md-3">
                            <label class="control-label">Số lượng<b style="color: red">*</b></label>
                            <input class="form-control" name="SoLuong" type="number" value="<?= $data_product['SoLuong'] ?>">
                        ...
                        <div class="form-group col-md-10">
                            ...
                            <div id="myfileupload">
                                <input type="hidden" id="uploadfile" name="AnhDaiDien" value="<?= $data_product['AnhDaiDien'] ?>" />
                            </div>
                            <div id="thumbbox">
                                <img height="auto" class="img-cover" id="thumbimage" width="200px" alt="Thumb image" src="<?= $data_product['AnhDaiDien'] ?>" />
                        ...
                        <div class="form-group col-md-8">
                            ...
                            <input class="form-control" name="ManHinh" type="text" value="<?= $data_product['ManHinh'] ?>">
                        </div>
                        <div class="form-group col-md-6">
                            <label class="control-label">Hệ điều hành</label>
                            <input class="form-control" name="HDH" type="text" value="<?= $data_product['HDH'] ?>">
                        </div>
                        <div class="form-group col-md-6">
                            <label class="control-label">Camera sau</label>
                            <input class="form-control" name="CamSau" type="text" value="<?= $data_product['CamSau'] ?>">
                        </div>
                        <div class="form-group col-md-6">
                            <label class="control-label">Cammera trước</label>
                            <input class="form-control" name="CamTruoc" type="text" value="<?= $data_product['CamTruoc'] ?>">
                        </div>
                        <div class="form-group col-md-6">
                            <label class="control-label">CPU</label>
                            <input class="form-control" name="CPU" type="text" value="<?= $data_product['CPU'] ?>">
                        </div>
                        <div class="form-group col-md-3">
                            <label class="control-label">Ram</label>
                            <input class="form-control" name="Ram" type="text" value="<?= $data_product['Ram'] ?>">
                        </div>
                        <div class="form-group col-md-3">
                            <label class="control-label">Rom</label>
                            <input class="form-control" name="Rom" type="text" value="<?= $data_product['Rom'] ?>">
                        </div>
                        <div class="form-group col-md-3">
                            <label class="control-label">SDCard</label>
                            <input class="form-control" name="SDCard" type="text" value="<?= $data_product['SDCard'] ?>">
                        </div>
                        <div class="form-group col-md-3">
                            <label class="control-label">Pin</label>
                            <input class="form-control" name="Pin" type="text" value="<?= $data_product['Pin'] ?>">
                        </div>
                        <div class="form-group col-md-12">
                            <label class="control-label">Thông tin sản phẩm</label>
                            <textarea class="form-control" name="MoTa" id="summernote" value="<?= $data_product['MoTa'] ?>"></textarea>
                            <script>
                                CKEDITOR.replace('summernote');
                            </script>
                        </div>
                        <div class="form-group col-md-12 mt-5">
                            <!-- Thêm class mt-3 để tạo khoảng cách top -->
                            <button type="submit" class="btn" id="btnThem" disabled="disable">Cập nhật</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    function initialize() {
        updateBrandOptions(); // Gọi hàm updateBrandOptions
        displayUploadedImages(); // Gọi hàm displayUploadedImages
        validateForm() // Gọi hàm validateForm
    }
    ...
    // Biến fileImageArray
    let fileImageArray = [];
    const existingImagesFromPHP = <?php echo json_encode($data_image_product); ?>;
    if (existingImagesFromPHP) {
        // Chuyển đổi dữ liệu PHP sang JavaScript
        // Thêm dữ liệu từ PHP vào fileImageArray (nếu có)
        existingImagesFromPHP.forEach(image => {
            fileImageArray.push(image.AnhURL);
        });
    }
    console.log(fileImageArray.length)
    ...
    choiceFilemutiple.addEventListener("click", () => {
        const optionsImage = {
            maxFiles: 4 - fileImageArray.length,
            onFileUploadFinished(file) {
                fileImageArray.push(file.url);
    ...
    function displayUploadedImages() {
        ...
        uploadfilemutiple.value = JSON.stringify(fileImageArray);
    }
